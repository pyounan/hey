...
func (fdm *FDM) SendMessage(event_label string, req Request, items []fdm.POSLineItem) (fdm.Response, error) {
	if len(items) == 0 {
		return Response{}, nil
	}
	...
	log.Println("=============================")
	msg := hashAndSignMsg(fdm.RCRS, event_label, t)
	...
	pf_response := Response{}
	stringRes := pf_response.Process(res, t)
	err = CheckError(pf_response)
	if err != nil {
		return pf_response, err
	}

	go func(event_label string, t Ticket, stringRes map[string]interface{}) {
		ej.Log(event_label, t, stringRes)
	}(event_label, t, stringRes)
